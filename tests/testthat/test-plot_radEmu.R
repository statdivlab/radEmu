...
test_that("plot_radEmu returns data frame and plot", {
  plot_out <- plot_radEmu(x = fitted_model)
  expect_true(is.data.frame(plot_out$data))
  expect_true(all(sapply(plot_out$plots, ggplot2::is.ggplot)))
})


test_that("plot_radEmu returns error when plot_key does not match coefficient table", {
  expect_error({
    plot_radEmu(x = fitted_model,
                plot_key = list(c("First Covariate" = "covariate1")))
  })
})

test_that("plot_radEmu returns error when coefficient is included multiple times in plot_key", {
  expect_error({
    plot_radEmu(x = fitted_model,
