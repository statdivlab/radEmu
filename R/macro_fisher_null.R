...
macro_fisher_null <- function(
  ...
  sm_denom <- as.numeric(as.matrix(
    1 + Matrix::crossprod(v, info_inverse) %*% v
  ))
  ...
  lag_deriv <- lapply(1:J, function(j) {
  ...
  while (!accept) {
    ...
    prop_gap <- constraint_fn[[k_constr]](prop_B[k_constr, ]) -
      prop_B[k_constr, j_constr]
    ...
    prop_lag_val <- do.call(
      sum,
      lapply(1:J, function(j) {
        -Y[, j, drop = FALSE] *
          prop_log_means_by_j[[j]] +
          exp(prop_log_means_by_j[[j]])
      })
    ) +
      u * prop_gap +
      (rho / 2) * prop_gap^2
