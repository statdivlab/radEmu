...
micro_score_stat <- function(Y,
  ...
  null_index <- (j - 1)*p + k
  A_11 <- as.matrix(A[-null_index,-null_index])
  A_21 <- as.matrix(A[null_index,-null_index,drop = FALSE])
  C <- cbind(-A_21%*%MASS::ginv(A_11),1)

  return( t((C%*%U))%*%solve(C%*%UUT%*%t(C))%*%(C%*%U))
